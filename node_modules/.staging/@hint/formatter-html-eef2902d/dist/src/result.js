...
const thirdPartyServices = load_json_file_1.default(path.join(__dirname, 'configs', 'third-party-service-config.json'));
const categoryImages = load_json_file_1.default(path.join(__dirname, 'configs', 'category-images.json'));
...
class HintResult {
    constructor(name, status, url, isScanner) {
        ...
        this.status = status;
        this.count = 0;
        this.thirdPartyInfo = thirdPartyServices[name] ? lodash_1.cloneDeep(thirdPartyServices[name]) : null;
...
class CategoryResult {
    constructor(name, url, isScanner) {
        this.cache = new Map();
        this.hints = [];
        this.passed = [];
        this.name = name;
    ...
    addProblem(problem) {
        ...
        if (!hint) {
            const status = problem.severity === types_1.Severity.error ? 'error' : 'warning';
            hint = new HintResult(hintId, status, this.url, this.isScanner);
            this.hints.push(hint);
        }
        if (problem.severity === types_1.Severity.error || problem.severity === types_1.Severity.warning) {
            this.hintsCount++;
        }
